...
import { billBelongsToClient, ifFetchStream } from "@/lib/interfast";
...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const owns = await billBelongsToClient(params.id, verified.payload.clientId);
  ...
  return new Response(upstream.body, {
    ...
    headers: {
      ...
      "Content-Disposition": `${dl ? "attachment" : "inline"}; filename="facture-${params.id}.pdf"`,
